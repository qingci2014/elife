...
const audioVersionsPath = path.join(siteRoot, "app", "generated", "audio-versions.ts");
const audioRoot = path.join(siteRoot, "public", "audio");
...
async function generateAudioVersions() {
  const versions = {};
  for (let number = 1; number <= 96; number += 1) {
    const book = Math.ceil(number / 32);
    const lessonDirectory = path.join(
      audioRoot,
      `book-${String(book).padStart(2, "0")}`,
      `lesson-${String(number).padStart(2, "0")}`,
    );
    const files = ["manifest.json", "dialogue.wav"];
    if (await pathExists(path.join(lessonDirectory, "transfer.wav"))) files.push("transfer.wav");
  ...
  const source = `// Generated by scripts/generate-content.mjs. Do not edit directly.\n\n` +
    `export const audioVersions: Record<number, string> = ${JSON.stringify(versions, null, 2)};\n`;
  await fs.mkdir(path.dirname(audioVersionsPath), { recursive: true });
  await fs.writeFile(audioVersionsPath, source, "utf8");
}

await generateAudioVersions();
